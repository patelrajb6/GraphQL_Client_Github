...
object Main extends App with LazyLogging {
  ...
  val topicdemo= new QueryBuilder()
    ...
    .getname().setStarGrazers(new StargazerConnection(first = 100)
    .gettotalCount().setNodeList(new User().getlogin())))
    .build()
  print(topicdemo)
  val repoDemo= new QueryBuilder()
    .setRepositoryQuery(new Repository("patelrajb6","Baccarat_Game")
      .getname().getDescription()).build()
  val ownerdemo= new QueryBuilder()
    .setRepositoryOwnerQuery(new RepositoryOwner("patelrajb6")
      .setRepository(new Repository("Baccarat_Game")
        .getcreatedAt().getname().getDescription().gethasIssuesEnabled()))
    .build()
 // println(ownerdemo)
 // println(repoDemo)
